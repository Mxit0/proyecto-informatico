...
class CompatibilityService {
  ...
  async enrichProductData(items) {
    ...
    for (const item of items) {
      ...
      if (!productDetails) {
        console.warn(
          `⚠️ Producto con ID ${item.product_id} no pudo ser enriquecido y fue saltado.`
        );
  ...
  async callGeminiAPI(items) {
    ...
    try {
      ...
      throw new Error(
        "Fallo al conectar con la API de Gemini. Verifique la clave y el servicio."
      );
    }
  }
...
`;
    ...
    items.forEach((item) => {
      prompt += `- ${item.name} (${item.category}): ${JSON.stringify(
        item.specifications
      )}\n`;
    });
...
Responde EXCLUSIVAMENTE en formato JSON con esta estructura:
{
  ...
  "pairs": [
    {
      "a": number,           // índice del primer componente en el arreglo 'items' (0-based)
      "b": number,           // índice del segundo componente en el arreglo 'items' (0-based)
      "compatible": boolean, // si el par es compatible
      "explanation": string  // explicación técnica breve del porqué
    }
  ]
}

Además, devuelve ejemplos concretos dentro del JSON cuando sea necesario. Usa índices basados en el orden del arreglo 'items'.
